...
- 설계 문서: `docs/00_사업수행계획서_분석.md`(사업 컨텍스트) → `DESIGN.md`(아키텍처) → `docs/01_기획서.md`, `docs/02_데이터분석.md`, `docs/03_화면설계.md`, `docs/05_VWorld_API_활용계획.md`(지도 API)
...
- 목표: UNE DS 기반 대시보드 — ①상황입력 패널(키워드 Chip 포함) ②지도 통합 뷰어(**VWorld 2D 지도** 타일 + Leaflet, L1~L4, 라이트=Base/다크=midnight) ③유사사례 Top-K 목록 ④상세조회(위험지구·하천) ⑤정합성(Citation·요인 기여도) + 판단기준 탭, 로컬 폰트, 라이트/다크
