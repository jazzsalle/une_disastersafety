...
- [ ] chunks.jsonl에 문서 6종(source_asset_id 6개: 구미·의왕·남원 저감계획, 요천·구미천·안양천 하천기본계획)의 청크가 모두 존재. 남원 admin_code=45190(전라북도), 안양천 청크에 plan_version 필드 존재
...
- [ ] `data/manual/rivers.json`에 요천·구미천·안양천 제원과 산정지점별 계획홍수량 + 홍수특보 기준유량(요천 Y4=2,005㎥/s→주의보 1,003/경보 1,404, 구미천 GM00=390㎥/s→195/273, 안양천 의왕구간 315㎥/s(금회 2020)→157.5/220.5 — plan_version으로 기수립 270㎥/s와 구분)
- [ ] `data/manual/geo.json`(또는 geo 산출)에 지자체 3곳 경계와 위험지구 대표 좌표(L1)·하천 구간(L2) GeoJSON 존재 — 경계·좌표 확보는 VWorld 2D데이터·지오코더 API 활용(docs/05, 결과 캐시 커밋 가능), 키 없이도 검증 가능해야 함
...
- [ ] POST /api/chat → UNI RAG main chat API 프록시(services/uni_rag.py 어댑터: /auth/login JWT 캐시·401 재로그인·SSE 중계). `UNI_RAG_ACCOUNT` 미설정 시 mock 폴백 응답(mode 필드로 구분) — 테스트는 mock 경로로 검증, 자격증명·JWT가 프론트 응답에 미노출
- [ ] `python -m pytest` 전체 통과 (검색 필터·ask mock·근거 제시율·chat mock 폴백·스키마 테스트 포함)
...
- [ ] ② 지도 통합 뷰어: **VWorld 2D 지도 타일**(Base/midnight, `VITE_VWORLD_API_KEY`) 기반 Leaflet 지도에 지자체 경계(L3)·위험지구 마커(L1)·하천 구간(L2) 표출, 좌표 불명확 항목은 목록 폴백(L4). OSM 등 다른 배경지도 미사용, VWorld 출처 표기 존재
...
- [ ] ⑥ 챗봇 대화 패널: /api/chat 연동(SSE 스트리밍 표시), 근거응답(ask)과 탭/모드 전환, mock 모드 배지 표기, 대화 이력 유지(history 전달)
