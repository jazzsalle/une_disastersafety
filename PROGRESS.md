...
- **docs/09_POC2_설계브리핑.md**: 최초 POC1 기반 브리핑으로 작성했다가 **전면 재작성** — POC2는 POC1과
  별개의 신규 설계(ChatGPT에서 거의 완성)라는 확인에 따라 POC1 내용 전부 제거, "T3Q 구축 방향 요약 +
  설계 점검 체크리스트 7항 + 보완 요청 프롬프트"만 남김. 업로드 금지 대상은 인증키·자격증명뿐,
  POC1 문서(docs/00~07)는 혼동 방지 위해 미첨부 권고
