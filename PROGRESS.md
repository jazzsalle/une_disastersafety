...
- (해소됨 2026-07-23) 남원 저감계획·안양천 하천기본계획 HWP → PDF 변환 확보, 분석 완료 — 코퍼스 6종으로 확대. 잔여 미확보: 남원 부록 A·B(HWP), 안양천 기타사항(스캔 이미지)

## Notes
- VWorld 개발키 발급·API 신청 완료(모바일·3D 데스크톱 제외 전 API), 등록 도메인 localhost 단독 — 키는 로컬 .env에만 있음(git 미포함). 집 등 다른 환경에서는 .env.example 복사 후 동일 키 재기입 필요 (docs/05 참조)
- 남원시는 전라북도(45190) — 원시자료 폴더명 "전라남도"는 오기(태깅 시 교정)
- 안양천 수치는 기수립(2015, 270㎥/s)/금회(2020, 315㎥/s) 이중 체계 — plan_version 메타로 구분
